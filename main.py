...
class StudyTrackerApp:
    ...
    def _create_gui(self):
        ...
        left_frame = tk.Frame(
            main_container, bg=Constants.WHITE, relief=tk.RAISED, borderwidth=2
        )
        left_frame.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=(0, 10))

        self.input_form = StudyInputForm(
            left_frame, on_save=self._save_session, on_clear=self._clear_form
        )
    ...
    def _show_weekly_summary(self):
        ...
        if not weekly_sessions:
            messagebox.showinfo(
                "Weekly Summary", "No study sessions in the past 7 days!"
            )
            return
        ...
        stats = {
            ...
            "avg_productivity": self.data_manager.get_average_productivity(
                weekly_sessions
            ),
